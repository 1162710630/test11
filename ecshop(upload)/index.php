...
if (!$smarty->is_cached('index.dwt', $cache_id))
{
    ...
    $smarty->assign('recommend',  goods_recommend());
...
// 物理删除 用sql(delete)删除
// 逻辑删除 用sql(update)更新
function goods_recommend() {
    $sql = "select g.goods_id,g.goods_name ,g.shop_price,g.market_price,g.promote_price,"
            . "g.goods_thumb,b.brand_name from ecs_goods g left join ecs_brand b on g.brand_id=b.brand_id where g.is_recommend=1 and g.is_on_sale=1 and g.is_delete=0 and g.is_alone_sale=1 order by"
            . " g.goods_id desc limit 0,4";
//     $db $GLOBALS['db'] 把外部变量$db声明成全局变量
//    global $db;
