...
class cls_template
{
    ...
    function fetch_str($source)
    {
        ...
        if(preg_match_all('~(<\?(?:\w+|=)?|\?>|language\s*=\s*[\"\']?php[\"\']?)~is', $source, $sp_match))
        {
         ...
         return preg_replace_callback("/{([^\}\{\n]*)}/",  function ($matchs){return $this->select($matchs[1]); }, $source);
    }
...
//                    $out = "<?php \n" . '$k = ' . preg_replace_callback("/(\'\\$[^,]+)/" , "stripslashes(trim('\\1','\''));", var_export($t, true)) . ";\n";
    ...
    function get_val($val)
    {
        ...
        {
            $val = preg_replace_callback("/\[([^\[\]]*)\]/",function($dog){ return '.'.str_replace('$','\$',$dog[1]);}, $val);
        }
...
//            $replacement = "'{include file='.strtolower('\\1'). '}'";
            $source      = preg_replace_callback($pattern, function($cat){return '{include file='.strtolower($cat[1]). '}';}, $source);
